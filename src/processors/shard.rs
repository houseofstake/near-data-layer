...
pub fn process_shard(
    ...
    // Build a mapping from receipt IDs to transaction hashes
    let mut tx_hash_map: HashMap<String, String> = HashMap::new();

    // Process transactions in the chunk to build receipt->transaction hash mapping
    if let Some(chunk) = &shard.chunk {
        for tx_with_outcome in &chunk.transactions {
            if let Some(transaction) = &tx_with_outcome.transaction {
                let tx_hash = if let Some(hash) = &transaction.hash {
                    hex::encode(&hash.bytes)
                } else {
                    continue; // Skip if no transaction hash
                };

                // Map receipts from this transaction's execution outcome
                if let Some(outcome_with_receipt) = &tx_with_outcome.outcome {
                    if let Some(execution_outcome) = &outcome_with_receipt.execution_outcome {
                        if let Some(outcome) = &execution_outcome.outcome {
                            // Map all receipt IDs generated by this transaction
                            for receipt_id in &outcome.receipt_ids {
                                let receipt_id_hex = hex::encode(&receipt_id.bytes);
                                tx_hash_map.insert(receipt_id_hex, tx_hash.clone());
                            }
                        }
                    }
                }
        ...
        if let Some(receipt) = &receipt_exec_outcome.receipt {
            ...
            if let Some(receipt::Receipt::Action(_)) = &receipt.receipt {
                ...
                if should_process_receipt(receipt) {
                    ...
                    // Get transaction hash for this receipt
                    let transaction_hash = tx_hash_map.get(&receipt_id).cloned().unwrap_or_default();

                    // Process receipt
                    process_receipt(changes, receipt, header, shard, &receipt_id, author, &transaction_hash);
